...
**The larger bin size plots hide the fact that the majority of flights actually have a slight delay, which is easily visible in the smallest bin size plot.**
...
**There were 68 flights in February headed to SFO**
```{r e_2}
...
**The distribution of arrival delays (`arr_delay`) is unimodal and right skewed. Due to the skewness, we will prefer the *median* (-5 min) and *IQR* (31 min) as measures of central tendency, as these measures are less sensitive to skew than mean / standard deviation.**
```{r e_3}
...
**DL and UA tie for the highest arr_delay IQR (22 minutes), but given UA has an extreme outlier (200 min), I would describe DL as the most generally variable (even though IQR is robust to outliers, this is a reasonable tie-breaker). **
```{r e_4}
...
**Choosing a month based on the mean would be reasonable if the distribution was relatively normal, and there were no extreme outliers, as the mean is sensitive to skew & outliers.**
...
**I would choose LGA as it has the highest rate of on-time departures.**
...
**As expected, we see a positive relationship between `avg_speed` and `distance`**
```{r e_8}
...
**Defining "arriving on time" as `arr_delay` <= 0, the corresponding max `dep_delay` observation is approximately 60 minutes.**
