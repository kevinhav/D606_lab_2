...
**KH: The smaller bin widths offer a much more detailed view of the distribution of flights and their departure time. The smallest bin width histogram shows us that while most flights leave on time, a decent amount can either leave early or slightly late. This is not as obvious with the larger bin sizes.**
...
**KH: There were 68 flights in February headed to SFO**
```{r e_2}
# Create a filtered dataframe for SFO flights in February
sfo_feb_flights <- nycflights %>%
  filter(dest == "SFO", month == 2)

# Count how many flights
sfo_feb_flights |> 
  summarize(n())
```
...
**KH: The distribution of arrival delays (`arr_delay`) is unimodal and right skewed. Due to the skewness, we will prefer the *median* (-5 min) and *IQR* (31 min) as measures of central tendency, as these measures are robust to skew, compared to the mean and standard deviation.**
```{r e_3}
# Plot histogram of arrival
ggplot(nycflights, aes(x = arr_delay)) +
  geom_histogram(binwidth = 10)

# Calculate measures of central tendency
arr_delay_metrics <- nycflights |> 
...
**KH: DL and UA tie for the highest arr_delay IQR (22 minutes), but given UA has an extreme outlier (200 min), I would describe DL as the most generally variable (even though IQR is robust to outliers, this is a reasonable tie-breaker). **
```{r e_4}
# Calculate median and IQR for arr_delay per carrier
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_ad = median(arr_delay), iqr_ad = IQR(arr_delay), n_flights = n())

# Plot histogram to visualize outliers
ggplot(sfo_feb_flights, aes(x = arr_delay, fill = carrier)) +
...
**KH: Choosing a month based on the mean would be reasonable if the distribution was relatively normal, and there were no extreme outliers, as the mean is sensitive to skew & outliers.**
**The median, in contrast, is robust to skewness and outliers, and would be a better choice if either were present in the data.**
...
**KH: I would choose LGA as it has the lowest proportion of delayed departures.**
...
nycflights <- nycflights |> 
  mutate(avg_speed = distance / (air_time / 60)) # Convert given air_time minutes to hours
```
...
**KH: As expected, we see a positive relationship between `avg_speed` and `distance`**
```{r e_8}
...
**KH: Defining "arriving on time" as `arr_delay` <= 0, the corresponding max `dep_delay` observation is approximately 60 minutes.**
**This indicates that departure delays greater than 60 minutes are very likely to delay expected arrival time.**
```{r e_9}
# Filter for AA, DL, and UA
aa_dl_ua_flights <- nycflights |> 
  filter(carrier %in% c('AA', 'DL', 'UA'))

# Create requested scatterplot of dep_delay vs arr_delay by carrier
ggplot(aa_dl_ua_flights, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point()

# Confirm the max dep_delay given arr_delay <=0 
aa_dl_ua_flights |> 
  filter(arr_delay <= 0) |> 
